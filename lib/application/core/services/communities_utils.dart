...
Future<Map<String, dynamic>> fetchRoomPowerLevels(
  String roomID,
  IGraphQlClient client,
) async {
  final Response response = await client.callRESTAPI(
    endpoint: roomPowerLevelsEndpoint(roomID),
    method: httpPOST,
    variables: <String, dynamic>{},
  );

  // TODO(abiud): process response
  final dynamic data = await json.decode(utf8.decode(response.bodyBytes));

  return data as Map<String, dynamic>;
}

/// Fetches the latest state of the API for the logged in user
///
/// We will do a full sync for now
///
/// [SyncParams] will be used to configure the sync
